...
import { threeContainer, web3DObj } from "./components/Experience";
...
const App: React.FC = () => {
  ...
  const colplate = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className="h-screen w-full">
      <Scene hovered={hovered} setHoverred={setHovered} color={color} />
      ...
      <div className="h-full w-full absolute pointer-events-none top-0 left-0 p-10">
        ...
        <div
          ref={colplate}
          className="shadow-inner shadow-zinc-300 absolute opacity-0 sm:h-fit sm:w-[17vw] w-[50%] h-fit left-1/2 -translate-x-1/2 sm:left-[10%] top-[15%] rounded-lg flex flex-col items-center justify-start sm:justify-between bg-zinc-50 sm:top-1/2 -translate-y-1/2 p-5"
        >
          <h1 className="font-[reg] font-bold text-center pb-5">CHOOSE YOUR PRESET</h1>
          <div className="w-full h-fit flex flex-wrap items-center justify-center gap-3">
            {colors.map((item, index) => (
              <div
                onClick={() => setColor(item.col)}
                key={index}
                className="shadow-md shadow-zinc-400 pointer-events-auto cursor-pointer sm:min-h-[2vw] sm:min-w-[4vw] h-[3vh] w-[5vh] rounded-tl-xl rounded-br-xl transition-all duration-300 hover:border-[3px] hover:border-white"
